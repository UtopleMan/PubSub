...
internal sealed class RabbitMqFireAndForgetPublisher<T> : IFireAndForgetPublish<T>, IAsyncDisposable
    where T : class
{
    ...
    private async ValueTask<IChannel> GetOrOpenChannelAsync(CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            await _channel.ExchangeDeclareAsync(
                ...
                cancellationToken: cancellationToken).ConfigureAwait(false);
            _logger.LogDebug("Fire-and-forget publisher channel opened for {RoutingKey}", _topic.RoutingKey);
            return _channel;
        }
        finally
        {
